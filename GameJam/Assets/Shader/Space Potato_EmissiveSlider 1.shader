...
Shader "Space Potato Panner" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _Parameter; uniform float4 _Parameter_ST;
...
///////// Gloss:
                float4 _Parameter_var = tex2D(_Parameter,TRANSFORM_TEX(node_9404, _Parameter));
                float gloss = _Parameter_var.g;
...
////// Specular:
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_9404, _MainTex));
                float3 diffuseColor = _MainTex_var.rgb; // Need this for specular when using metallic
                float specularMonochrome;
                float3 specularColor;
                diffuseColor = DiffuseAndSpecularFromMetallic( diffuseColor, _Parameter_var.b, specularColor, specularMonochrome );
...
////// Emissive:
                ...
                float3 emissive = (_Emissive_var.rgb*_EmissivePower);
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 specColor;
                float4 _Parameter_var = tex2D(_Parameter,TRANSFORM_TEX(node_9404, _Parameter));
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, _Parameter_var.b, specColor, specularMonochrome );
                float roughness = 1.0 - _Parameter_var.g;
